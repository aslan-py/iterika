...
def _build_prompt(products: list[Product], stats: PriceStats) -> str:
    """Строит гибридный промпт: цена как база + подъём флагманов."""
    ...
    return (
        ...
        f'Ценовые границы сегментов (по данной выборке):\n'
        f'  Эконом:    до {stats.p_low:.0f} руб\n'
        f'  Стандарт:  {stats.p_low:.0f} – {stats.p_high:.0f} руб\n'
        f'  Премиум:   от {stats.p_high:.0f} руб\n\n'
        ...
        f'Товары:\n{body}\n\n'
        f'Ответь JSON: {{"segments": [...]}} — '
        f'ровно {count} строк в том же порядке.\n'
